...
std::vector<ItemType> &AVLTree::addVector (vector<ItemType> &vec, string item, FieldSearch field, SearchMode mode)
{
...
AVLTree &AVLTree::addTree (AVLTree &tree, string item, FieldSearch field, SearchMode mode)
{
    if ( this->root == nullptr ) throw ContactDatabase::ExEmptyTree();
...
bool AVLTree::remove(ItemType existingData) {
    if ( this->root == nullptr ) return false;
...
ItemType &AVLTree::locate(ItemType existingData) {
    if ( this->root == nullptr ) throw ContactDatabase::ExEmptyTree();
    ...
    while ( child != nullptr ) {
        ...
        if ( existingData == node->data ) return node->data;
    }
    
    throw ContactDatabase::ExItemNotFound(existingData);
}
...
AVLTree &AVLTree::addTree (AVLNode *node, AVLTree &tree, string item,FieldSearch field, SearchMode mode)
{
    if ( node != nullptr )
    {
        if (node->data.getFirstName() != "NULL")
...
void AVLTree::preorder(stringstream& out, AVLNode* node) const
{
    if ( node == nullptr ) return;
...
void AVLTree::inorder(stringstream& out, AVLNode* node) const {
    if ( node == nullptr ) return;
...
void AVLTree::postorder(stringstream& out, AVLNode* node) const {
    if ( node == nullptr ) return;
